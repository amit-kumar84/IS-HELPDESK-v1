...
<h2 height="73" colspan="2" style="text-align:center; font-size: 20px;text-shadow: 1px 1px 2px black, 0 0 25px blue, 0 0 5px darkblue; color:#B1FF10;">&nbsp; Verified Asset List &nbsp;</h2>
<div>
		<?php
			$query_sel = mysqli_query($link,"SELECT `CATG`, COUNT(CATG), CATG_TYPE FROM `hardware_master` WHERE USERNAME!='ISKOT' AND USG!='MS (STANDBY)' AND USG!='WO' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND `issued_on`!='0' GROUP BY CATG");
      ...
      <tbody>
		...
		$s_no = 1 ;
			...
			if(isset($sub))
			{
				...
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND `DEPT`='$dept_wise_search' AND `SEC`='$sec_wise_search' AND `CATG`='$catg_wise_search' ORDER BY issued_on DESC");
				}
				// Y N N
				else if($dept_wise_search=="All" && $sec_wise_search!= "All" && $catg_wise_search!="All")
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND `SEC`='$sec_wise_search' AND `CATG`='$catg_wise_search' ORDER BY issued_on DESC");
				}
				// Y Y N
				else if($dept_wise_search=="All" && $sec_wise_search== "All" && $catg_wise_search!="All")
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND CATG!='PROJECTOR' AND `CATG`='$catg_wise_search' ORDER BY issued_on DESC");
				}
				// Y N Y
				else if($dept_wise_search=="All" && $sec_wise_search!= "All" && $catg_wise_search=="All")
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND `SEC`='$sec_wise_search' ORDER BY issued_on DESC");
				}
				// N Y Y
				else if($dept_wise_search!="All" && $sec_wise_search== "All" && $catg_wise_search=="All")
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND `DEPT`='$dept_wise_search' ORDER BY issued_on DESC");
				}
				// N N Y
				else if($dept_wise_search!="All" && $sec_wise_search!= "All" && $catg_wise_search=="All")
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND `DEPT`='$dept_wise_search' AND `SEC`='$sec_wise_search' ORDER BY issued_on DESC");
				}
				// N Y N
				else if($dept_wise_search!="All" && $sec_wise_search== "All" && $catg_wise_search!="All")
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' AND `DEPT`='$dept_wise_search' AND `CATG`='$catg_wise_search' ORDER BY issued_on DESC");
				}
				// Y Y Y
				else if($dept_wise_search=='All' && $sec_wise_search=='All' && $catg_wise_search=='All')
				{
		  $hardware_master_sel=mysqli_query($link,"SELECT * FROM `hardware_master` WHERE `issued_on`!='0' AND USG!='WO' AND USG!='MS (STANDBY)' AND USERNAME!='ISKOT' AND CATG!='WEB_CAM' AND CATG!='PROJECTOR' ORDER BY issued_on DESC");
				}
        ...
        <tr style="text-align: left; font-size:14px" id="row_hov">
		  ...
		  <td><?php echo $hardware_id = $hardware_master_arr["HD_ID_NO"] ; ?></td>
		  <td><?php echo $hardware_master_arr["DEPT"] ." ( " .$hardware_master_arr["SEC"] ." ) " ; ?></td>
          <td><?php echo $hardware_master_arr["RFID_NO"] ; ?></td>
