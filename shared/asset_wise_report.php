<div>
	<h2 height="73" colspan="2" style="text-align:center; font-size: 36px;text-shadow: 1px 1px 2px black, 0 0 25px blue, 0 0 5px darkblue; color:#B1FF10;">&nbsp; PC / Laptop Wise Report &nbsp;</h2>
	
<!--***********************************************************************************************************************************************************************
																	Search PC wise Report
*************************************************************************************************************************************************************************-->
  
	<form id="form" action="" name="form1" method="POST">
	  <p>
		  <b style="color:green;">PC / Laptop Wise Report : </b>
			<select name="asset_wise_search" id="call_timing_dept" required>
				<option style="color:#AFAFAF;" value="" selected disabled>Select Asset</option>
					<option value="ALL">All</option>
					<?php						
					$asset_id_fetch = mysqli_query($link,"SELECT DISTINCT(HD_ID_NO) FROM `hardware_master` WHERE CATG='PC' || CATG='LAPTOP' ORDER BY HD_ID_NO ASC");
						while($asset_id = mysqli_fetch_array($asset_id_fetch))
						{
					?>
					<option><?php echo $asset_id["HD_ID_NO"] ; ?></option>
					<?php
						}
					?>
	...
	<table width="100%" height="54" border="1" cellpadding="1" cellspacing="0" style="float:left;" id="table_func">
		...
		$s_no=1;
			...
			if(isset($sub))
			{
				...
				else if($asset_wise_search=='')
				{
					echo "<b style='color:red;'><sup>*</sup>Please select Asset ID first.</b> <br/><br/>" ;
				}
				else if($asset_wise_search!='ALL' || $asset_wise_search!='')
				{
					$query_sel=mysqli_query($link,"SELECT * FROM `complain_register` WHERE `pc_no`='$asset_wise_search' ORDER BY substring(t_no,1,5) DESC, substring(t_no,8,12) ");
					$total_row_count = mysqli_num_rows($query_sel);
						echo "<b style='color:#3795F8;'>Total Result :</b> " .$total_row_count ."<br><br>" ; // print per page call record 
						
						echo "<b style='color:green; font-size:30px;'><center><u>" .$asset_wise_search ."</u></center></b></br>" ;
				}
				else
				{
					echo "0 Record Found!" ;
		...
		?>
			<tr style="text-align: center; font-size:10px" id="row_hov">
				<td><?php echo $s_no ; ?></td>
				<td><b><?php echo $call_arr["pc_no"] ; ?></b></td>
