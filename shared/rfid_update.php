			...
			if($usr_staffid != "")
			{
				...
				$usage = $u_usg ;
					if($u_usg == 'OTHER')
					  ...
					  if(mysqli_query($link,"UPDATE `hardware_master` SET `STAFF_NO`='$usr_staffid', `USERNAME`='$u_user_name', `DEPT`='$s_deptt', `SEC`='$s_sec', `USG`='$usage', `IP_ADDRESS`='$ip_address',`RFID_NO`='$rfid_tag_no', `issued_on`=NOW()
    ...
    <tbody>
      ...
      <tr>
		...
		<select name="u_usg" style="width: 173px;" onchange='check_u_usg(this.value);' required="required">
			...
			<option value="SAP / COMMON">SAP / COMMON</option>
			<option value="SAP / APPRENTICE">SAP / APPRENTICE</option>
			<option value="NET / COMMON">NET / COMMON</option>
      ...
      <tr>
        <td height="35" style="text-align: left"><strong>RFID TAG Number</strong></td>
        <td><input name="rfid_tag_no" type="text" value="" placeholder=" RFID TAG NUMBER" size="30"></td>
      </tr>
      <tr>
        <td height="35" style="text-align: left"><strong>IP Address / USB</strong></td>
        <td><input name="ip_address" type="text" value="" placeholder=" IP ADDRESS" size="30"></td>
      </tr>
